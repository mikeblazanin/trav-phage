...
# my.lwd = 2
# 
# ##need to add sd lines
# 
# for (j in 1:length(unique(mean_rates$Proj))) {
#   my.filename <- paste("avg_treats_", j, ".tiff", sep = "")
#   tiff(filename = my.filename, width = 13, height = 7, units = "in",
#        compression = "none", res = 300)
#   my.mar <- par()$mar
#   par(mar = my.mar + c(1, 3, 0, 0))
#   
#   my.proj <- unique(mean_rates$Proj)[j]
#   sub_mean_rates <- subset(mean_rates, mean_rates$Proj == my.proj)
#   
#   for (i in 1:length(unique(sub_mean_rates$Treat))) {
#     my.sub = subset(sub_mean_rates, sub_mean_rates$Treat == unique(sub_mean_rates$Treat[i]) & as.numeric(sub_mean_rates$Time) <= 14)
#     my.sub <- my.sub[order(as.numeric(my.sub$Time)), ]
#     if (i == 1) {
#       plot(my.sub$Time, my.sub$`Mean Rate`, type = "o", 
#            ylim = c(min(as.numeric(subset(sub_mean_rates, as.numeric(sub_mean_rates$Time) <= 14)$`Mean Rate`)), 
#                     max(as.numeric(subset(sub_mean_rates, as.numeric(sub_mean_rates$Time) <= 14)$`Mean Rate`))),
#            ann = F, col = my_colors[i], pch = my.pch,
#            lwd = my.lwd, cex = 2.5, cex.axis = 2.5)
#     } else {
#       lines(my.sub$Time, my.sub$`Mean Rate`, type = "o", col = my_colors[i], pch = my.pch,
#             lwd = my.lwd, cex = 2.5)
#     }
#     lines()
#   }
#   legend(x = 10.5, y = 0.028, legend = unique(sub_mean_rates$Treat), pch = my.pch, col = my_colors,
#          lwd = my.lwd, cex = 2.5)
#   mtext("Mean Migration Rate (cm/hr)", side = 2, line = 4, cex = 2.5)
#   mtext("Transfer", side = 1, line = 4, cex = 2.5)
#   dev.off()
# }
# 
# 
...
# 
# 
# my.sub = subset(end_data, end_data$Time == 14)
# 
# # #make stripchart of T14 (last shared timepoint)
# # stripchart(my.sub$`Rate (cm/hr)` ~ my.sub$Treat, method = "overplot", vertical = T)
# 
# #ANOVA on T14 data
# my.anova <- aov(my.sub$`Rate (cm/hr)` ~ my.sub$Treat)
# summary(my.anova)
# 
# my_colors <- c("red", "orange", "green", "blue", "yellow")
# 
# #make plot of ea treat to check if pops are stable position
# for (i in 1:length(unique(end_data$Treat))) {
#   tiff(filename = paste(unique(end_data$Treat)[i], "by_pop.tiff", sep = "_"), 
#        width = 13, height = 7, units = "in",
#        compression = "none", res = 300)
#   par(mar = my.mar + c(1, 3, 0, 0))
#   treat.sub = subset(end_data, end_data$Treat == unique(end_data$Treat)[i] &
#                        as.numeric(end_data$Time) <= 14)
#   for (j in 1:length(unique(treat.sub$Rep))) {
#     my.sub = subset(treat.sub, treat.sub$Rep == unique(treat.sub$Rep)[j])
#     my.sub <- my.sub[order(as.numeric(my.sub$Time)), ]
#     if (j == 1) {
#       plot(my.sub$Time, my.sub$`Rate (cm/hr)`, type = "o", 
#            ylim = c(min(as.numeric(subset(end_data, as.numeric(end_data$Time) <= 14)$`Rate (cm/hr)`)), 
#                     max(as.numeric(subset(end_data, as.numeric(end_data$Time) <= 14)$`Rate (cm/hr)`))),
#            ann = F, col = my_colors[j], pch = my.pch,
#            lwd = my.lwd, cex = 2, cex.axis = 2)
#       mtext("Migration Rate (cm/hr)", side = 2, line = 4, cex = 2.5)
#       mtext("Transfer", side = 1, line = 4, cex = 2.5)
#       #main = unique(end_data$Treat)[i]
#     } else {
#       lines(my.sub$Time, my.sub$`Rate (cm/hr)`, type = "o", col = my_colors[j], pch = my.pch,
#             lwd = my.lwd, cex = 2)
#     }
#   }
#   dev.off()
...
# #make plot of ea treat but against day not transfer #
# for (i in 1:length(unique(end_data$Treat))) {
#   treat.sub = subset(end_data, end_data$Treat == unique(end_data$Treat)[i])
#   for (j in 1:length(unique(treat.sub$Rep))) {
#     my.sub = subset(treat.sub, treat.sub$Rep == unique(treat.sub$Rep)[j])
#     my.sub <- my.sub[order(as.Date(my.sub$timestamp)), ]
#     if (j == 1) {
#       plot(my.sub$timestamp, my.sub$`Rate (cm/hr)`, type = "o", 
#            ylim = c(min(as.numeric(end_data$`Rate (cm/hr)`)), max(as.numeric(end_data$`Rate (cm/hr)`))),
#            ylab = "Rate (cm/hr)", xlab = "Date", col = my_colors[j], pch = my.pch,
#            #xlim = as.Date(c("2017-04-19", "2017-05-10")))
#            #xlim = as.Date(c(min(end_data$timestamp), max(end_data$timestamp))))
#            lwd = my.lwd, main = unique(end_data$Treat)[i])
#     } else {
#       lines(my.sub$timestamp, my.sub$`Rate (cm/hr)`, type = "o", col = my_colors[j], pch = my.pch,
#             lwd = my.lwd)
#     }
#   }
# }
# 
# plot(end_data$`Rate (cm/hr)` ~ as.Date(end_data$timestamp))
# plot(end_data$`Rate (cm/hr)` ~ end_data$timediff)
# summary(lm(end_data$`Rate (cm/hr)` ~ end_data$timediff))
