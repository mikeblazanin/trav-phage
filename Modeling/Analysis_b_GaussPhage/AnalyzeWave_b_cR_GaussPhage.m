...
FileList = dir('../Outputs_b_GaussPhage/OutputsbcR/*.mat');
...
for ii = 1:length(FileList)
    File = ['../Outputs_b_GaussPhage/OutputsbcR/', FileList(ii).name];
    load(File)
...
%     xline(WaveStartInd2)
    ...
    %% Store measurements
    Measurements(ii, :) = [cell_population, cell2_population, relativecR, Relativeb, SimParams.cR, SimParams.irate];
end
