...
FileList = dir('../Outputs_b_GaussPhage/OutputsbChi/*.mat');
...
for ii = 1:length(FileList)
    File = ['../Outputs_b_GaussPhage/OutputsbChi/', FileList(ii).name];
    load(File)
...
%     xline(WaveStartInd2)
    ...
    %% Store measurements
    Measurements(ii, :) = [cell_population, cell2_population, relativeChi, Relativeb, SimParams.Chi, SimParams.irate];
end
