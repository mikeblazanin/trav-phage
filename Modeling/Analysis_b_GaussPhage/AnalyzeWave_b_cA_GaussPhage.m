...
FileList = dir('../Outputs_b_GaussPhage/OutputsbcA/*.mat');
...
for ii = 1:length(FileList)
    File = ['../Outputs_b_GaussPhage/OutputsbcA/', FileList(ii).name];
    load(File)
...
%     xline(WaveStartInd2)
    ...
    %% Store measurements
    Measurements(ii, :) = [cell_population, cell2_population, relativecA, Relativeb, SimParams.cA, SimParams.irate];
end
