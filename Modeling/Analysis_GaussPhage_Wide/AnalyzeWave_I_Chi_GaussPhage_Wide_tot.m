...
FileList = dir('../Outputs_WidePhageControl/OutputsIChi_GaussPhage/*.mat');
...
for ii = 1:length(FileList)
    File = ['../Outputs_WidePhageControl/OutputsIChi_GaussPhage/', FileList(ii).name];
    load(File)
...
saveas(gcf, './I_vs_Chi_gPhage_Wide_tot.png')

%% Export Data
headers = ["Cell_population", "Cell2_population", "relativeChi", "relativeI", "Chi", "irate"];
textHeader = strjoin(headers, ',');
fid = fopen('./I_vs_Chi_gPhage_Wide_tot.csv', 'w');
fprintf(fid,'%s\n',textHeader)
fclose(fid)

dlmwrite('./I_vs_Chi_gPhage_Wide_tot.csv',Measurements,'-append');
