...
FileList = dir('../Outputs/OutputsIcR/*.mat');
...
for ii = 1:length(FileList)
    File = ['../Outputs/OutputsIcR/', FileList(ii).name];
    load(File)
...
saveas(gcf, './I_vs_cR_tot.png')

%% Export Data
headers = ["Cell_population", "Cell2_population", "relativecR", "relativeI", "cR", "irate"];
textHeader = strjoin(headers, ',');
fid = fopen('./I_vs_cR_tot.csv', 'w');
fprintf(fid,'%s\n',textHeader)
fclose(fid)

dlmwrite('./I_vs_cR_tot.csv',Measurements,'-append');