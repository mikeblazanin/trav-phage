...
FileList = dir('../Outputs_b_NoPhage/OutputsIb/*.mat');
...
for ii = 1:length(FileList)
    File = ['../Outputs_b_NoPhage/OutputsIb/', FileList(ii).name];
    load(File)
...
%     xline(WaveStartInd2)
    ...
    %% Store measurements
    Measurements(ii, :) = [cell_population, cell2_population, relativeI, Relativeb, SimParams.Y, SimParams.b];
end
