%% calculate the chemotactic coefficients based on the PLOS Comp. 
 function [wavespeed final_profile dyn_profile dx]=chemo_oxygen_fun_v2(Asp,Time)
...
ux=ux./sum(ux);
u(:,2:round(DL)+1)=P'*ux*500*dCWBias; %unit is OD1,~6*8.4*1.4*dx cells in 600um*14um*dx %total cell: 0.7*10^9*(600*14*10^4+900*14*900+?)*10^-12=15~20*10^4 cells
u(:,1)=u(:,2);
