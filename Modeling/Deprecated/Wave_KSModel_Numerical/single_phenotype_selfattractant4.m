%% calculate the chemotactic coefficients based on the PLOS Comp. 
 function [wavespeed_late final_profile dyn_profile dx]=single_phenotype_selfattractant4(Asp,Time,GSfacter,betafacter)
...
% GS=8*4/60*10;
beta0=9.3*0.84/60/6;
beta=beta0*betafacter;%betafactor=1;
  T=round(Time/dt);%
...
DS=500/dx^2;%http://compost.css.cornell.edu/oxygen/oxygen.diff.water.html
G0=9.3*0.84/60;% 10uM/min for OD 1 cells.
GS=G0*GSfacter;%GSfactor=0.18;
...
%     Gamma1=G0*u1.*(0.1+0.9./(1+(u1/9.5).^3)).*(A1>0.001);
    ...
    Gamma1=G0*sum(u1)./(1+(0.5./A1));%.*((kaO./(1+((Oex-S1)/ktO).^5))+1-kaO);
    GammaS=GS*sum(u1)./(1+(kA./A1)); Gamma_beta=beta.*S1;
   

    %f=ki'*A1;
     f=ki'*log((1+A1/Ki+S1/KiS1)./(1+A1/Ka+S1/KaS1));%
     fS=kiS'*(log((1+S1/KiS1)./(1+S1/KaS1)))*0;%-log((1+S1/KiO2)./(1+S1/KaO2)));
    ...
    %f=ki'*A1;
    f=ki'*log((1+A1/Ki+S1/KiS1)./(1+A1/Ka+S1/KaS1));%
    fS=kiS'*(log((1+S1/KiS1)./(1+S1/KaS1)))*0;
    g(:,2:end-1)=u1(:,2:end-1).*(f(:,3:end)-f(:,1:end-2))/2/dx;   g(:,1)=-g(:,2);g(:,end)=-g(:,end-1);
      gS(:,2:end-1)=u1(:,2:end-1).*(fS(:,3:end)-fS(:,1:end-2))/2/dx;   gS(:,1)=-gS(:,2);gS(:,end)=-gS(:,end-1);
    ...
    Sb(2:end-1)=DS*(S1(1:end-2)+S1(3:end)-2*S1(2:end-1))+GammaS(2:end-1)-Gamma_beta(2:end-1);
