%% calculate the chemotactic coefficients based on the PLOS Comp. 
 function [wavespeed final_profile dyn_profile dx]=chemo_oxygen_fun(Asp,Time)
...
G0=9*0.84/60;% 10uM/min for OD 1 cells.
DL=800/dx; V=800/dx;
for x=1:round(V)
    ux(x)=1;
end
ux=ux./sum(ux);
u(:,2:round(DL)+1)=P'*ux*1500*dCWBias; %unit is OD1,~6*8.4*1.4*dx cells in 600um*14um*dx %total cell: 0.7*10^9*(600*14*10^4+900*14*900+?)*10^-12=15~20*10^4 cells
u(:,1)=u(:,2);
A=Asp*(1-1./(1+(LX*1000/(V*dx*2.8)).^4));
...
%     Gamma1=G0*u1.*(0.1+0.9./(1+(u1/9.5).^3)).*(A1>0.001);
    Gamma1=G0*sum(u1)./(1+(5./A1)).*((kaO./(1+((Oex-O1)/ktO).^3))+1-kaO);
