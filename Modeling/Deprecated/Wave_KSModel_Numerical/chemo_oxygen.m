...
G0=8*0.84/60;% 10uM/min for OD 1 cells.
DL=1000/dx; V=800/dx;
for x=1:round(DL)
    ux(x)=exp(-(x)^2/V^2);
...
ux=ux./sum(ux);
u(:,2:round(DL)+1)=P'*ux*2000*dCWBias; %unit is OD1,~5*dx cells in 600um*10um*dx %total cell: 0.7*10^9*600*10*10^4*2*10^-12=8*10^4 cells
u(:,1)=u(:,2);
A=Asp*(1-1./(1+(LX*1000/(V*dx*2.2)).^3.5));
