...
close all;
    Asp=[200];
     Time=[30];tic;
 uiopen('D:\Dropbox\Yale\paper_wave_diversity\others\Wave_KSModel_Numerical\comparision.fig',1)
...
tic;
 ...
 Time0=0;
for j=1:length(Asp)
        [final_profile(j),wavespeed(j) dyn_profile(j)]=chemo_oxygen_fun_v5(Asp(j),Time(j)*60,Time0*60);
%        cell_density=sum(final_profile(j).cell_density);
%         x=0:dx:dx*(length(cell_density)-1);
%         cell_density=sum(final_profile(j).cell_density);
%          [maxden maxind]=max(cell_density(40:end));
% %          subplot(2,1,1);
%           cell_density1=sum(final_profile(j).cell_density(1:180,:));
%           cell_density2=sum(final_profile(j).cell_density(181:end,:));
%          plot(x-x(maxind+39),cell_density1(1:end)*8.4*10^8,'Color','r');hold on;xlim([-1.5 1.5]);
%           plot(x-x(maxind+39),cell_density2(1:end)*8.4*10^8,'Color','r');
% %         subplot(2,1,2);
...
%         [maxden maxind]=max(cell_den(120:end));
%         cell_den_shifted(:,i-5)=cell_den(maxind+119-125:maxind+119+125);
% 
% %         cell_den_shifted(:,i)=cell_den(x-x(maxind+47)>-2&x-x(maxind+47)<4);
...
%         cell_den_mean(:,j)=mean(cell_den_shifted');
%         cell_den_mean_norm(:,j)=cell_den_mean(:,j)./max(cell_den_mean(:,j));
%         clear cell_number cell_den_shifted;
%         
%   end
% x=(1:length([maxind+119-125:maxind+119+125]))*dx-125*dx;
% plot(x/1000,cell_den_mean*8.4*10^8);
%   xlim([-1.5 1.5]);
% 
%    %%
% % %     for j=1:3
...
% uiopen('D:\Dropbox\Yale\paper_wave_diversity\others\Wave_KSModel_Numerical\tb_comparison.fig',1)
% 
...
%     [maxden maxind]=max(cell_density(120:end));
%     dyn_den_end=final_profile(i).cell_density(:,120+maxind-150:end);
...
%  T=length(peakpos);
% p=polyfit(time(peakpos>6.65),peakpos(peakpos>6.65),1);
% p(1)*60
% x=0:dx:(650-1)*dx;
% x=x-(denmaxind-1)*dx; 
