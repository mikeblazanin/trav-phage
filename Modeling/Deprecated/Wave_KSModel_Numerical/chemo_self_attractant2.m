%% calculate the chemotactic coefficients based on the PLOS Comp. 
 function [final_profile wavespeed dyn_profile dx]=chemo_self_attractant2(Asp,Time,GSfactor,betafactor)
...
dCWBias=0.005;CWBias=0.005:dCWBias:CWBias_exp(end);
...
G0=9.3*0.84/60*0.4;% 9uM/min for OD 1 cells.
GS=G0*GSfactor;%GSfactor=0.18;
beta0=G0;
...
ua=u;ub=u;uc=u;ud=u;Aa=A;Ab=A;Ac=A;Ad=A;Oa=O;Ob=O;Oc=O;Od=O;Sa=S;Sb=S;Sc=S;Sd=S;


%%
tt=1;
g=u;grad=A;gO=u;gS=u;
%%
for t=1:T
    u1=u;A1=A;A1(A1<=0)=0;
    O1=O; O1(O1<=0)=0;
    ...
    S1=S; S1(S1<=0)=0;
    GammaS=GS*sum(u1)./(1+(0.5./A1)); Gamma_beta=beta.*S1;
    f=ki'*log((1+A1/Ki)./(1+A1/Ka));f=ki'*log((1+A1/Ki+S1/KiS1)./(1+A1/Ka+S1/KaS1));
    ...
    Sa(2:end-1)=DS*(S1(1:end-2)+S1(3:end)-2*S1(2:end-1))+GammaS(2:end-1)-Gamma_beta(2:end-1);
    ...
    Sb(2:end-1)=DS*(S1(1:end-2)+S1(3:end)-2*S1(2:end-1))+GammaS(2:end-1)-Gamma_beta(2:end-1);    Ob(2:end-1)=DO*(O1(1:end-2)+O1(3:end)-2*O1(2:end-1))-GammaO(2:end-1)+KO*((Oex-O1_sense(2:end-1))+abs(Oex-O1_sense(2:end-1)))/2;
    Ab(1)=Ab(2); Ab(end)=Ab(end-1);    
    ub(:,1)=ub(:,2);ub(:,end)=ub(:,end-1); 
    Ob(1)=Ob(2); Ob(end)=Ob(end-1);  
    ...
    Sc(2:end-1)=DS*(S1(1:end-2)+S1(3:end)-2*S1(2:end-1))+GammaS(2:end-1)-Gamma_beta(2:end-1);
    ...
    Oc(1)=Oc(2); Oc(end)=Oc(end-1);   
    Sc(1)=Sc(2); Sc(end)=Sc(end-1);  
    %%
    ...
    Sd(2:end-1)=DS*(S1(1:end-2)+S1(3:end)-2*S1(2:end-1))+GammaS(2:end-1)-Gamma_beta(2:end-1);
    ...
    Od(1)=Od(2); Od(end)=Od(end-1);
    Sd(1)=Sd(2); Sd(end)=Sd(end-1);   
