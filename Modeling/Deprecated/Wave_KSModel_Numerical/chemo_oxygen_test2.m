...
Timeind=log(18):(log(55)-log(20))/100:log(55);
Asp=exp(Aspind);Time=exp(Timeind);tic;
...
%  save asp_range.mat -v7.3
%%
dx=40;
for i=1:length(Asp)
    
      cell_density(i,:)=sum(final_profile(i).cell_density);
    [denmax(i),denmaxind(i)]=max(cell_density(i,100:end));
...
%     end
     ...
     wavedensity1(i)=sum(cell_density(i,(100+denmaxind(i)-100):end));
...
figure;
plot(Asp(Asp>20&Asp<310),(cell_number1(Asp>20&Asp<310)));
figure;
plot(cell_number2(Asp>40&Asp<310),smooth(Asp(Asp>40&Asp<310).*wavespeed_late(Asp>40&Asp<310)./cell_number2(Asp>40&Asp<310))*10^-18*600*14*60/1000,'r-');


 plot(Asp(Asp>40&Asp<310),cell_number2(Asp>40&Asp<310));
 xlim([20,250]);
 %%
dCWBias=0.001; CWBias=0.001:dCWBias:0.99;P = lognpdf(CWBias,log(0.318),0.298)/sum(lognpdf(CWBias,log(0.318),0.298))/dCWBias;
 for i=1:length(Asp)
    dyn_den_end=dyn_profile(i).cell_density(:,100+denmaxind(i)-100:100+denmaxind(i)+40,end);
