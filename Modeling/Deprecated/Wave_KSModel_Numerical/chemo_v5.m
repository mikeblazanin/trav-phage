...
%         cell_den=reshape(cell_den,4,length(cell_den)/4);cell_den=mean(cell_den);
        ...
        cell_den_shifted(:,i-5)=cell_den(maxind+47-53:maxind+47+60);
...
x=(1:length([maxind+47-53:maxind+47+60]))*dx-50*dx;
...
dx=50;
for i=1:length(Asp)
    cell_number(i)=sum(cell_den_mean(:,i))*10^-12*600*14*dx*8.4*10^8;
end
...
%     [maxden maxind]=max(cell_density(48:end));
%     dyn_den_end=final_profile(i).cell_density(:,48+maxind-50:end);
