%% calculate the chemotactic coefficients based on the PLOS Comp. 
 function [final_profile wavespeed dyn_profile dx]=chemo_oxygen_fun_v4(k,Asp,Time,Time0)
...
a=Yp/alpha;
Drot=0.038*2;%adam's data
v=28*4/pi;%2D run speed 28um/s
theta=1-0.1564;%from adam's paper
lambdaRdiff=-(lambdaR).*a.*(a-1).*(alpha*g2*Kd)./(2*(Kd+alpha*a).^2); 
N=6;
% tau=10*exp(-((CWBias-0.15)./0.16).^2).*(CWBias<0.24)+(CWBias>=0.24).*0.002.*CWBias.^-5.7;
tau=15*exp(-2.2*CWBias);
% tau=5;
ki=theta.*v.^2.*N.*tau.*(lambdaRdiff./(1+lambdaR./lambdaT)./(lambdaR.*theta+Drot))./(1+tau.*(lambdaR.*theta+Drot))/3;
...
G0=9*0.84/60;% 9uM/min for OD 1 cells.
DL=800/dx; 
V=370/dx;
for x=1:round(2*DL)
    ux(x)=exp(-(x)^2/(k*DL)^2);
