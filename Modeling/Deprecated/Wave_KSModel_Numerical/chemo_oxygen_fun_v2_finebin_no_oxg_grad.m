%% calculate the chemotactic coefficients based on the PLOS Comp. 
 function [final_profile wavespeed dyn_profile dx]=chemo_oxygen_fun_v2_finebin_no_oxg_grad(k,Asp,Time)
...
dCWBias=0.005;CWBias=0.01:dCWBias:CWBias_exp(end);
...
alpha=5.1;%adam's data
a=Yp/alpha;
Drot=0.075*2;%adam's data
 tauR=1./(Drot+lambdaR);
v=26*4/pi;%2D run speed 28um/s
theta=1-cos(81/180*pi);%from adam's paper
  Diff=v^2*exp(-8.25*CWBias);
%   Diff=2275*exp(-34*CWBias)+172*exp(-3.3*CWBias);
%    Diff=v^2*(1-CWBias).*tauR/3/theta;
lambdaRdiff=-(lambdaR).*a.*(a-1).*(alpha*g2*Kd)./(2*(Kd+alpha*a).^2); 
tau=50*exp(-7*CWBias);
N=6;
% ki=10^6*exp(-10*CWBias)+4600;
%  tau=0;
...
%     Gamma1=G0*u1.*(0.1+0.9./(1+(u1/9.5).^3)).*(A1>0.001);
    Gamma1=G0*sum(u1)./(1+(kA./A1)).*(kaO+(O/ktO).^kappa)./(1+(O/ktO).^kappa);%(kaO./(1+((Oex-O)/ktO).^5))+1-kaO;
...
%      gO(:,2:end-1)=u1(:,2:end-1).*(fO(:,3:end)-fO(:,1:end-2))/2/dx;   gO(:,1)=-gO(:,2);gO(:,end)=-gO(:,end-1);
    ...
    Gamma1=G0*sum(u1)./(1+(kA./A1)).*(kaO+(O/ktO).^kappa)./(1+(O/ktO).^kappa);
