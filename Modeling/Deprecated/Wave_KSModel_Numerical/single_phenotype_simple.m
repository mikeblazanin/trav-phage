%% calculate the chemotactic coefficients based on the PLOS Comp. 
 function [wavespeed_late final_profile dyn_profile dx]=single_phenotype_simple(Asp,Time,TB)
%  dCWBias=0.005; CWBias=0.01:dCWBias:0.99;P = lognpdf(CWBias,log(0.295),0.308)/sum(lognpdf(CWBias,log(0.295),0.308))/dCWBias;
dCWBias=0.001; P = [0.5 0.5]/dCWBias;%lognpdf(CWBias,log(0.318),0.298)/sum(lognpdf(CWBias,log(0.318),0.298))/dCWBias;
%CWBias=[0.3357 0.3357];
CWBias=[TB TB];
...
DO=600/dx^2;%http://compost.css.cornell.edu/oxygen/oxygen.diff.water.html
