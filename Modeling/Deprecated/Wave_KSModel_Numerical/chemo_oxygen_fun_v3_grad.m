...
tau=5*CWBias+6.45;%*exp(-2.86*CWBias); %Diversity in adapation time from Park & Cluzel - Nature (2010)
N=6;
%  tau=0;
...
u(:,1)=u(:,2);
A=Asp*(1-1./(1+(LX*1000/(DL*dx*2.3)).^3));
...
%   Gamma1=G0*u1.*(0.1+0.9./(1+(u1/9.5).^3)).*(A1>0.001);
    Gamma1=G0*sum(u1)./(1+(0.5./A1)).*((kaO./(1+((Oex-O1)/ktO).^H))+1-kaO);
%     Gamma1=G0*sum(u1)./(1+(0.5./A1)).*(-kaO*(1/250*(250-O1)).^H+1);
