%% calculate the chemotactic coefficients based on the PLOS Comp. 
 function [final_profile wavespeed dyn_profile dx]=chemo_self_attractant(Asp,Time,GSfactor,betafactor)
...
G0=9.3*0.84/60/2;% 9uM/min for OD 1 cells.
...
for t=1:T
    ...
    S1=S; S1(S1<=0)=0;
    GammaS=GS*sum(u1)./(1+(0.5./A1)); Gamma_beta=beta.*sum(u1)./(1+(0.5./S1))./(1+A1./kA);
