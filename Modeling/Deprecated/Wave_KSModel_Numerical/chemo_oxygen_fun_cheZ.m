%% calculate the chemotactic coefficients based on the PLOS Comp. 
 function [wavespeed_late final_profile dyn_profile dx]=chemo_oxygen_fun_cheZ(Asp,Time)
...
P1 = lognpdf(CWBias,log(0.285),0.3)/sum(lognpdf(CWBias,log(0.285),0.3))/dCWBias;
P2 = lognpdf(CWBias,log(0.36),0.315)/sum(lognpdf(CWBias,log(0.36),0.315))/dCWBias;
...
P=[P1 P2]/2;
con_cw=sum(PWT.*CWBias)./sum(P.*CWBias);
...
%        end%plot(n,'g');
 ut(:,:,tt)=u;
        ...
        Ot(:,tt)=O;
         subplot(2,1,1)
        plot(LX,sum(u(1:length(CWBias)/2,:))/max(sum(u(:,:))),'b');hold on;
         plot(LX,sum(u(length(CWBias)/2+1:end,:))/max(sum(u(:,:))),'b--');hold on;
%         plot(LX,A/max(A),'r--'); plot(LX,O/max(O),'g--'); plot(LX,G0./(1+0.5./A).*(0.2+(O1/80).^2)./(1+(O1/80).^2)/G0,'c');
            subplot(2,1,2)
        plot(LX,sum(u(:,:)),'b');hold on;
        plot(LX,sum(u(1:length(CWBias)/2,:)),'r');hold on;
         plot(LX,sum(u(length(CWBias)/2+1:end,:)),'g');hold on;
%           toc
%      end
     end
end
