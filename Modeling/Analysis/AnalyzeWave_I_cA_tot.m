...
FileList = dir('../Outputs/OutputsIcA/*.mat');
...
for ii = 1:length(FileList)
    File = ['../Outputs/OutputsIcA/', FileList(ii).name];
    load(File)
...
saveas(gcf, './I_vs_cA_tot.png')

%% Export Data
headers = ["Cell_population", "Cell2_population", "relativecA", "relativeI", "cA", "irate"];
textHeader = strjoin(headers, ',');
fid = fopen('./I_vs_cA_tot.csv', 'w');
fprintf(fid,'%s\n',textHeader)
fclose(fid)

dlmwrite('./I_vs_cA_tot.csv',Measurements,'-append');
