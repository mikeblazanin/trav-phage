...
FileList = dir('../Outputs_b_GaussPhage/OutputsbY/*.mat');
...
for ii = 1:length(FileList)
    File = ['../Outputs_b_GaussPhage/OutputsbY/', FileList(ii).name];
    load(File)
...
%     xline(WaveStartInd2)
    ...
    %% Store measurements
    Measurements(ii, :) = [cell_population, cell2_population, relativeY, Relativeb, SimParams.Y, SimParams.irate];
end
